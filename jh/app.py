...
# .env 파일에서 환경 변수(OPENAI_API_KEY)를 로드하는 로직이 추가됨
...
# --- dotenv 로드 (로컬 환경에서 .env 파일 사용 시 필요) ---
try:
    load_dotenv()
except ImportError:
    pass
...
def get_openai_client():
    try:
        ...
        st.error(
            f"❌ OpenAI 클라이언트 초기화 실패: {e}. .env 파일 또는 환경 변수를 확인해주세요."
        )
        st.stop()
...
def call_llm_for_kpi_summary(df_influencers: pd.DataFrame, w_config: dict) -> str:
    ...
    위 데이터를 분석하여, **'현재 전략에 따른 마케팅 효율성 진단'**과 **'향후 캠페인 방향성에 대한 핵심 제언'**을 포함하는 보고서를 작성해 주세요.
    """
...
def load_data():
    try:
        ...
        df_influencers = pd.read_sql(query_influencers, engine)
        # 누락된 컬럼 추가
        ...
        st.error(
            "데이터 파일을 찾을 수 없습니다. 'data_generator.py'를 실행하여 파일을 먼저 생성해 주세요."
        )
...
def kpi_dashboard_module(df_influencers: pd.DataFrame):
    st.title("📊 캠페인 성과 분석 대시보드 및 전략 가중치 설정")
    st.markdown(
        "KPI 정의를 확인하고, 인플루언서 매칭에 반영될 **전략적 성과 가중치**를 조정합니다. **가중치에 따라 순위가 변경**됩니다."
    )
    st.markdown("---")

    col_def, col_set = st.columns([1, 1])

    with col_def:
        st.header("1. 핵심 KPI 정의 (성과 지표)")
        st.markdown("마케팅 효율을 객관적으로 측정하는 지표들입니다.")

        st.subheader("💰 비용 효율 지표")
        st.latex(
            r"CPM = \frac{\text{총 비용}}{\text{총 노출수}} \times 1,000 \quad \Rightarrow 1,000\text{회 노출당 비용}"
        )
        st.latex(
            r"CPR = \frac{\text{총 비용}}{\text{총 도달수}} \quad \Rightarrow \text{순수 고객 1명 도달당 비용}"
        )
        st.latex(
            r"CPA = \frac{\text{총 캠페인 비용}}{\text{순수 전환수}} \quad \Rightarrow \text{고객 1명 획득당 비용}"
        )

        st.subheader("📈 고객 반응 지표")
        st.latex(
            r"ER = \frac{\text{좋아요} + \text{댓글} + \dots}{\text{도달수}} \times 100 \quad \Rightarrow \text{도달 고객당 참여율}"
        )
        st.metric(
            label="평균 도달수 (Reach)",
            value=f"{df_influencers['평균_도달수'].mean():,.0f} 명",
            help="캠페인에 노출된 순수 사용자 수의 평균",
        )

    with col_set:
        st.header("2. 전략적 성과 가중치 설정")
        st.markdown(
    ...
    st.header("3. 전략적 가중치 기반 인플루언서 효율성 순위 (Top 10)")
    ...
    # --- 4. GenAI KPI 요약 보고서 ---
    st.header("4. 📝 전체 KPI 요약 및 마케팅 전략 제언")
...
def app():
    st.set_page_config(
        page_title="K-Beauty 인플루언서 마케팅 대시보드 (GPT-4o 연동)",
        layout="wide"
    )
