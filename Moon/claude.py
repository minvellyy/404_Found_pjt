...
{{
  "understood": "Brief summary in Korean",
  "follow_up_questions": [
    "Question 1 in Korean?",
    "Question 2 in Korean?"
  ],
  "initial_filters": {{
    "countries": [],
    "ages": [],
...
"""
    try:
        ...
        raise Exception(f"OpenAI Clarification API 오류: {e}")
...
"""
    try:
        ...
        raise Exception(f"OpenAI Refine API 오류: {e}")
...
"""
    try:
        ...
        raise Exception(f"OpenAI Brand Fit API 오류: {e}")
...
"""
    prompt += """
--- Task ---
Write a comprehensive Strategic Insight Report in Korean:
...
"""
    try:
        ...
        raise Exception(f"OpenAI Insight API 오류: {e}")
...
def render_sidebar_expert_mode(df):
    """전문가 모드 사이드바 (가중치 중심)"""
    st.sidebar.header("🔧 전문가 모드 (가중치 설정)")
    st.sidebar.markdown("---")
    st.sidebar.caption("💡 캠페인 목표에 맞춰 각 항목의 중요도 (가중치 1: 낮음 ~ 5: 매우 높음)를 설정하세요.")
    
    with st.sidebar.expander("🌍 국가 (Country)", expanded=True):
        st.slider("국가 가중치", 1, 5, st.session_state.country_weight, key='country_weight') 
        st.multiselect("Target Countries", ALL_COUNTRIES, key='target_countries')

    with st.sidebar.expander("🎂 연령 (Age)", expanded=True):
        st.slider("연령 가중치", 1, 5, st.session_state.age_weight, key='age_weight') 
        st.multiselect("Target Ages", ALL_AGES, key='target_ages',
                      format_func=lambda x: x.replace('_', ' ').title())

    with st.sidebar.expander("⚧️ 성별 (Gender)", expanded=False):
        st.slider("성별 가중치", 1, 5, st.session_state.gender_weight, key='gender_weight') 
        st.multiselect("Target Genders", ALL_GENDERS, key='target_genders')

    with st.sidebar.expander("🎨 관심사 (Interest)", expanded=False):
        st.slider("관심사 가중치", 1, 5, st.session_state.interest_weight, key='interest_weight') 
        st.multiselect("Target Interests", ALL_INTERESTS, key='target_interests')

    st.sidebar.markdown("---")
    st.sidebar.caption("💡 인플루언서의 신뢰도 및 기본 요건을 설정하세요.")

    with st.sidebar.expander("📊 신뢰도 (Reliability)", expanded=True):
        st.slider("참여율 가중치", 1, 5, st.session_state.engagement_weight, key='engagement_weight',
                  help="참여율(0~15%)이 높을수록 점수가 높아집니다.")
        st.slider("가짜 팔로워 가중치", 1, 5, st.session_state.fake_followers_weight, key='fake_followers_weight',
                  help="가짜 팔로워 비율(0~100%)이 낮을수록 점수가 높아집니다.")
            
    with st.sidebar.expander("📱 플랫폼 (Platform)", expanded=False):
        st.multiselect("Target Platforms", ALL_PLATFORMS_CHOICES, key='target_platforms',
                       default=st.session_state.target_platforms) 

    with st.sidebar.expander("👥 팔로워 수 (Followers)", expanded=False):
        col1, col2 = st.columns(2)
        with col1:
            st.number_input("최소 (명)", min_value=0, step=10000, key='min_followers', format="%d")
        with col2:
            st.number_input("최대 (명)", min_value=0, step=10000, key='max_followers', format="%d", value=1000000)

def render_step1_conversational():
    """1단계: AI와 대화형 캠페인 기획"""
    st.subheader("1. 🤖 AI 대화형 캠페인 기획")
    ...
    if not st.session_state.clarification_phase:
        st.markdown("💬 **어떤 인플루언서를 찾고 계신가요?** 자유롭게 설명해주세요.")
        st.text_area(
            "캠페인 목표를 입력하세요:",
            key='initial_campaign_goal',
            placeholder="예: 미국 30대 여성 타겟, K-뷰티 제품 홍보...",
            height=150
        )
        st.file_uploader("📎 참고 자료 (선택, 필터링 참고용)", type=["txt", "pdf"], key='filter_report_file',
                         help="시장 조사 보고서 등을 첨부하면 AI가 필터 설정 시 전략적 컨텍스트로 활용합니다.")

        if st.button("🚀 AI 분석 시작", type="primary", use_container_width=True):
            if not st.session_state.initial_campaign_goal.strip():
                st.error("캠페인 목표를 입력해주세요.")
            else:
                with st.spinner("🤖 GPT-4가 요청사항을 분석 중..."):
                    try:
                        ...
                        if uploaded_file:
                            if uploaded_file.type == "text/plain":
                                ...
                                filter_report_content = f"[{uploaded_file.name} 파일 업로드됨. 텍스트 파싱 생략.]"
    ...
    # Phase 2: AI 추가 질문 & 답변
        ...
        data = st.session_state.clarification_data
        st.success(f"✅ **AI가 이해한 내용:** {data['understood']}")
        st.markdown("---")

        if data.get('follow_up_questions'):
            st.info("🤖 **더 정확한 검색을 위한 추가 질문** (선택사항)")
            for i, question in enumerate(data['follow_up_questions']):
                st.text_input(
                    f"**Q{i+1}.** {question}",
                    key=f'followup_{i}', 
                    placeholder="답변 입력 또는 비워두기..."
                )
            st.markdown("---")
            col1, col2 = st.columns(2)
            with col1:
                st.button("⚡ 건너뛰고 바로 검색", use_container_width=True,
                           help="AI가 추출한 최소 정보로 검색",
                           on_click=cb_skip_filters)
            with col2:
                st.button("✅ 답변 완료 & 정교한 검색", type="primary",
                           use_container_width=True,
                           help="답변을 반영하여 AI가 필터 정제",
                           on_click=cb_refine_filters)
        else:
            st.success("✨ 입력하신 내용이 충분히 명확합니다!")
            st.button("✅ 검색 시작", type="primary", use_container_width=True,
                      on_click=cb_skip_filters) 
    ...
    if st.session_state.get('filter_applied_success'):
        st.success("✅ 필터가 적용되었습니다! 왼쪽 사이드바에서 세부 가중치를 조정하세요.")
        with st.expander("📊 AI가 자동 설정한 필터 상세 보기"):
            col1, col2 = st.columns(2)
            with col1:
                st.write("**국가:**", f"{st.session_state.target_countries or '전체'} (가중치 {st.session_state.country_weight})")
                st.write("**연령:**", f"{st.session_state.target_ages or '전체'} (가중치 {st.session_state.age_weight})")
                st.write("**성별:**", f"{st.session_state.target_genders or '전체'} (가중치 {st.session_state.gender_weight})")
                st.write("**관심사:**", f"{st.session_state.target_interests or '전체'} (가중치 {st.session_state.interest_weight})")
            with col2:
                st.write("**플랫폼:**", f"{st.session_state.target_platforms or 'Any'}") 
                st.write("**팔로워:**", f"{st.session_state.min_followers:,} ~ {st.session_state.max_followers:,}")
                st.write("**참여율 가중치:**", f"{st.session_state.engagement_weight}")
                st.write("**가짜 팔로워 가중치:**", f"{st.session_state.fake_followers_weight}")
        st.session_state.filter_applied_success = False 

def render_step2_filtered_list(df):
    """2단계: 필터링된 인플루언서 목록"""
    st.subheader("2. 📋 필터링된 인플루언서 목록 (Top 100)")

    filtered_df = get_filtered_influencers(df)
    
    active_filters = []
    if st.session_state.target_countries: active_filters.append(f"국가 (W:{st.session_state.country_weight})")
    if st.session_state.target_ages: active_filters.append(f"연령 (W:{st.session_state.age_weight})")
    if st.session_state.target_genders: active_filters.append(f"성별 (W:{st.session_state.gender_weight})")
    if st.session_state.target_interests: active_filters.append(f"관심사 (W:{st.session_state.interest_weight})")
    active_filters.append(f"참여율 (W:{st.session_state.engagement_weight})") 
    active_filters.append(f"신뢰도 (W:{st.session_state.fake_followers_weight})") 
    sort_info = ["캠페인 적합도 점수 (가중치 반영)"]
    
    st.info(f"**적합도 점수 계산 항목:** {' | '.join(active_filters) or '기본 퀄리티 (참여율, 신뢰도)'}")
    st.caption(f"**정렬 기준:** {' > '.join(sort_info)} 순서로 우선순위 지정됨")

    st.metric("검색 결과", f"{len(filtered_df):,}명 / {len(df):,}명")

    if filtered_df.empty:
        st.warning("⚠️ 필터 조건에 맞는 인플루언서가 없습니다. 조건을 완화하거나 왼쪽 사이드바에서 수동 조정해보세요.")
        return None

    st.markdown("**✅ Select** 체크박스를 클릭하여 관심 있는 인플루언서를 'Like' 하세요:")

    col1, col2 = st.columns(2)
    with col1:
        if st.button("✅ 전체 선택 (Like All)", use_container_width=True):
            st.session_state.liked_influencers.update(filtered_df.head(100)['influencer_name'])
            st.rerun()
    with col2:
        if st.button("❌ 전체 해제 (Unlike All)", use_container_width=True):
    ...
    if 'Total_Fit_Score' in filtered_df.columns:
        filtered_df['적합도 점수 (100점)'] = np.round(filtered_df['Total_Fit_Score'], 1)
        cols_to_display.insert(1, '적합도 점수 (100점)') 
...
def render_step3_detail_analysis(influencer_data, df):
    """3단계: 상세 분석 및 비용 확인"""
    st.subheader(f"3.1 {influencer_data['influencer_name']} (기본 정보)")
    ...
    with st.expander("클릭하여 모든 분포 차트 확인"):
        ...
        create_altair_bar_chart(df_age['Percentage'] * 100, '연령 비율 (Percentage)', sort_order=ALL_AGES)
        ...
        create_altair_bar_chart(df_gender['Percentage'] * 100, '성별 비율 (Percentage)')

        st.markdown("##### 🌍 국가 분포 (Top 5)")
        ...
        if country_dist:
             ...
             create_altair_bar_chart(df_country['Percentage'] * 100, '국가 비율 (Top 5 Percentage)')
        else:
            st.write("국가 정보 없음")
        ...
        if interest_dist:
            df_interest = pd.DataFrame.from_dict(interest_dist, orient='index', 
                                               columns=['Percentage']).nlargest(5, 'Percentage')
            create_altair_bar_chart(df_interest['Percentage'] * 100, '관심사 비율 (Top 5 Percentage)')
        else:
            st.write("관심사 정보 없음")

    st.markdown("---")
    st.button(f"💔 '{influencer_data['influencer_name']}' Unlike",
              type="secondary",
              use_container_width=True,
...
def render_step4_ai_analysis(influencer_data):
    """4단계: AI 심층 분석"""
    name = influencer_data['influencer_name']
    st.subheader(f"4.1 {name} (AI 심층 분석)")

    persona_context = st.session_state.initial_campaign_goal

    st.markdown("#### 4.2 분석 맥락 제공")
    if persona_context:
        st.info(f"**적용된 캠페인 목표:**\n\n{persona_context}")
    else:
        st.warning("⚠️ 1단계에서 캠페인 목표를 입력하면 AI 분석 품질이 향상됩니다.")

    st.multiselect("A. 브랜드 핵심 키워드 (Soft Fit 평가용)",
                   options=ALL_INTERESTS,
                   key='brand_keywords_input_4',
                   help="d'Alba와 연관성 높은 키워드 선택")

    st.text_area("B. 브랜드 가이드라인 (선택)",
                 key='analysis_brand_guideline_input',
                 placeholder="예: d'Alba는 '우아함', '이탈리아', '비건'을 중요시...")

    st.file_uploader("C. 추가 참고 보고서 (분석용)", type=["txt", "pdf"], key='analysis_report_file',
                     help="브랜드 핏 및 인사이트 보고서 생성 시 전략적 컨텍스트로 활용됩니다.")

    if st.button("GPT-4로 브랜드 핏 평가", key="fit_btn_4"):
        brand_keywords = st.session_state.brand_keywords_input_4
        if not brand_keywords:
            st.error("브랜드 키워드를 1개 이상 선택해주세요.")
        elif not persona_context:
            st.error("캠페인 목표를 먼저 입력해주세요.")
        else:
                ...
                try:
                    ...
                    if uploaded_file:
                        if uploaded_file.type == "text/plain":
                            ...
                            analysis_report_content = f"[{uploaded_file.name} 파일 업로드됨. 텍스트 파싱 생략.]"
    ...
    if (st.session_state.brand_fit_result and
        ...
        st.metric("점수", f"{fit_data.get('brand_fit_score', 'N/A')} / 100")
        st.info(f"**AI 분석:** {fit_data.get('reason', 'N/A')}")
    else:
    ...
    if st.button("GPT-4로 통합 인사이트 보고서 생성", type="primary", use_container_width=True):
        if not persona_context:
            st.error("캠페인 목표를 먼저 입력해야 합니다.")
        else:
            with st.spinner("GPT-4 통합 분석 중..."):
                try:
                    filter_report = "N/A"
                    ...
                    if uploaded_filter_file:
                        if uploaded_filter_file.type == "text/plain":
                            ...
                            filter_report = f"[{uploaded_filter_file.name} 파일 업로드됨. 텍스트 파싱 생략.]"
                    ...
                    if uploaded_analysis_file:
                        if uploaded_analysis_file.type == "text/plain":
                            ...
                            analysis_report = f"[{uploaded_analysis_file.name} 파일 업로드됨. 텍스트 파싱 생략.]"
                    ...
                    st.success(insight) # 생성 직후 바로 표시
                except Exception as e:
                    st.error(f"인사이트 생성 오류: {e}")
...
def render_step5_contract_export(influencer_data, df):
    ...
    st.info(f"결재 요청할 최종 후보: {len(seeding_df)}명")

    # CSV 다운로드
    st.markdown("#### A. 최종 시딩 목록 (CSV)")
    ...
    if (st.session_state.insight_report and
        ...
        if uploaded_filter_file:
            ...
            else: filter_report = f"[{uploaded_filter_file.name} 파일 업로드됨. 텍스트 파싱 생략.]"
        
        analysis_report = "N/A"
        uploaded_analysis_file = st.session_state.get('analysis_report_file')
        if uploaded_analysis_file:
            if uploaded_analysis_file.type == "text/plain": analysis_report = uploaded_analysis_file.getvalue().decode("utf-8")
            else: analysis_report = f"[{uploaded_analysis_file.name} 파일 업로드됨. 텍스트 파싱 생략.]"
    ...
    st.text_input("1. 제안 비용 (예: $500 USD)", key='proposed_cost', value=suggested_cost) 
    st.caption(f"💡 AI 제안 금액: **{suggested_cost}** (CPM/Followers 기반)")
    
    st.text_input("2. 캠페인 기간 (예: 2025-12-01 ~ 2025-12-15)", key='campaign_period')
    st.text_area("3. 콘텐츠 가이드라인", key='content_guideline',
                 placeholder="예: 12월 10일까지 틱톡 영상 1건, 인스타 릴스 1건...")

    if st.button("AI로 계약서 초안 생성", type="primary", use_container_width=True):
        ...
        if not all([cost, period, guideline]):
            st.error("모든 항목을 입력해야 계약서를 생성할 수 있습니다.")
        else:
            with st.spinner("GPT-4가 계약서 초안 작성 중..."):
                try:
    ...
    if (st.session_state.generated_contract and
        ...
        st.download_button(
            f"✍️ {contract_name} 계약서 초안 PDF 다운로드",
            data=pdf_bytes,
...
def run_app(df):
    """메인 애플리케이션 실행"""
    st.title("d'Alba AI Seeding Platform 🚀")
    ...
    if not liked_list:
        st.info("💡 먼저 2단계에서 '✅ Select'로 관심 있는 인플루언서를 선택해주세요.")
        return

    st.markdown("### 3~5단계: 선택한 인플루언서 심층 분석")

    selected_name = st.selectbox(
        f"분석할 인플루언서 선택 (총 {len(liked_list)}명):",
        options=liked_list,
        key="analysis_selector",
        help="선택한 인플루언서의 상세 정보, AI 분석, 계약서를 확인할 수 있습니다."
    )
...
def main():
    ...
    tab_app, tab_data = st.tabs(["🚀 시딩 플랫폼", "⚙️ 데이터 관리"])
    ...
    try:
        ...
        st.session_state.font_error = "⚠️ PDF 한글 폰트 미설정\n'MALGUN.TTF' 파일 필요"

    with tab_data:
        st.subheader("📦 데이터 관리 (MySQL)")
        
        df = None # df 변수 초기화
        try:
            # [수정] DB에서 데이터 로드
            df = load_data() 
            st.success(f"✅ 'influencers_v25' 테이블 로드 완료 ({len(df):,}건)")

            col1, col2 = st.columns(2)
            with col1:
                st.metric("총 인플루언서", f"{len(df):,}명")
                st.metric("플랫폼", f"{df['platform'].nunique()}개")
            with col2:
                st.metric("평균 팔로워", f"{df['followers'].mean():,.0f}명")
                st.metric("평균 참여율", f"{df['engagement_rate_pct'].mean():.2f}%")

            if st.button("🔄 가상 데이터 재생성 (DB 덮어쓰기)", use_container_width=True):
                st.cache_data.clear() # 캐시 비우기
                st.cache_resource.clear() # DB 커넥션 캐시도 비우기
                create_mock_data() # [수정] DB에 새로 생성
                st.rerun()

            with st.expander("📊 데이터 미리보기 (DB)"):
                st.dataframe(df.head(20), use_container_width=True)

            with st.expander("📈 데이터 통계 (DB)"):
                st.write("**플랫폼 분포:**")
                create_altair_bar_chart(df['platform'].value_counts(), '플랫폼 분포')
                ...
                create_altair_bar_chart(follower_dist, '팔로워 분포')

        except Exception as e:
            # [수정] FileNotFoundError가 아닌 DB 연결/테이블 없음 오류 처리
            st.warning(f"⚠️ DB에서 'influencers_v25' 테이블을 찾을 수 없거나 오류가 발생했습니다.")
            st.error(f"오류 상세: {str(e)[:200]}...") # 실제 오류 메시지 표시
            st.info("아래 버튼을 눌러 DB에 가상 데이터를 생성하세요.")

            if st.button("📦 DB에 가상 데이터 생성하기 (10,000명)",
                          use_container_width=True,
                          type="primary"):
                st.cache_data.clear()
    ...
    with tab_app:
        if df is not None:
            ...
            if missing_cols:
                st.error(f"⚠️ 데이터 형식 오류. 누락된 컬럼: {missing_cols}")
                st.info("'⚙️ 데이터 관리' 탭에서 데이터를 재생성해주세요.")
            else:
                run_app(df)
        else:
            st.info("👈 먼저 '⚙️ 데이터 관리' 탭에서 데이터를 생성하거나 로드해주세요.")
            st.markdown("""
            ### 💡 사용 방법
            
            1. **데이터 관리** 탭에서 가상 데이터 생성 (DB)
            2. **AI 대화형 기획**으로 캠페인 목표 입력
            3. GPT-4가 자동으로 추가 질문 생성
            4. 답변하거나 건너뛰기로 검색
            5. 왼쪽 **전문가 모드**로 세부 조정
            6. 관심 있는 인플루언서 'Like'
            7. AI 심층 분석 및 계약서 생성
            ...
            - 🤖 **대화형 AI**: 자연어로 요구사항 입력
            - ⚡ **MySQL 연동**: 실시간 데이터베이스 연동
            - 🔧 **전문가 모드**: 가중치 기반 Top-K 랭킹
            - 💎 **브랜드 핏 분석**: GPT-4 기반 적합도 평가
            - 📄 **자동 계약서**: AI가 계약서 초안 자동 생성
            """)
